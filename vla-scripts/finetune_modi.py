...
class FinetuneConfig:
    ...
    log_freq: int = 10                               # Logging frequency in steps
...
def get_ckpt_dir(cfg):
    dir_str = get_run_id(cfg) + f"--{cfg.resume_step}_chkpt"
    ckpt_path = Path(cfg.run_root_dir) / dir_str
    return ckpt_path
...
def init_module(
    ...
    if cfg.resume:
        state_dict = load_checkpoint(module_name, get_ckpt_dir(cfg), cfg.resume_step)
...
def log_metrics_to_tensorboard(metrics, prefix, step, writer) -> None:
    """
    ...
    for name, value in metrics.items():
        ...
        if name == "loss_value":
            ...
            tag = f"{prefix}/{name.replace('_', ' ').title()}"
...
def save_training_checkpoint(
    ...
    if distributed_state.is_main_process:
        ...
        torch.save(optimizer_ckpt, checkpoint_dir / f"optimizer--{checkpoint_name_suffix}")
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.use_film:
        ...
        if cfg.resume:
            state_dict = load_checkpoint("vision_backbone", get_ckpt_dir(cfg), cfg.resume_step)
    ...
    scheduler = MultiStepLR(
        ...
        # last_epoch= cfg.resume_step if cfg.resume else -1,
    )
    if cfg.resume:
    ...
    # if cfg.resume:
    #     op_state_dict = load_checkpoint("optimizer", get_ckpt_dir(cfg), cfg.resume_step)
    #     optimizer.load_state_dict(op_state_dict['optimizer_state_dict'])
    #     scheduler.load_state_dict(op_state_dict['scheduler_state_dict'])  # 恢复 scheduler 状态
    ...
    # Start training
    with tqdm.tqdm(total=cfg.max_steps, leave=False, initial=cfg.resume_step if cfg.resume else 0) as progress:
        vla.train()
        optimizer.zero_grad()
        for batch_idx, batch in enumerate(dataloader):
            ...
            current_gradient_step_index = batch_idx // cfg.grad_accumulation_steps
            gradient_step_idx = current_gradient_step_index if not cfg.resume else cfg.resume_step + current_gradient_step_index

            # Compute smoothened train metrics
            smoothened_metrics = compute_smoothened_metrics(recent_metrics)

            # Push Metrics to TensorBoard (every log_freq gradient steps)
            ...
            if gradient_step_idx > 0 and log_step % cfg.save_freq == 0 and log_step != cfg.resume_step:
